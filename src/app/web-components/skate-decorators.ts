...
const IS_DEV = environment.production === false;
...
export function customElement( name: string ): ClassDecorator {
  return function <T extends Constructor<Component<any>>>( Target: T ): T {
...
export function shadyCssStyles() {
  return function <T extends Constructor<Component<any>>>( Target: T ) {
